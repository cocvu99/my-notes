...
![how system calls work](resources/images/100/kernel_and_user_mode.jpg)

```mermaid
flowchart TD
    subgraph USER["User space (Ring 3)"]
        APP["Applicationcalls printf("hi")"]
        LIBC["libc wrapperwrite(1, "hi", 2)"]
        APP --> LIBC
    end

    SYS["syscall instructionCPU switch: Ring 3 to Ring 0"]

    subgraph KERNEL["Kernel space (Ring 0)"]
        TABLE["Syscall tabledispatch by number in rax"]
        HANDLER["sys_write handlercopies buffer, does I/O"]
        TABLE --> HANDLER
    end
...
```mermaid
flowchart LR
    A["printf()user code"] --> B["Format engineparse %d, %s"]
    B --> C["stdio bufferin FILE struct"]
    C --> D["write() syscallonly on flush"]
